...
9. The LLM decision is authoritative for strategic judgment. Server-side validation must only enforce mechanical execution constraints: schema validity, reconciliation, market/outcome existence, disabled market families, `> 90%` entry cap, stake/cash caps, slot caps, duplicate-open protection, and hard daily cooldown. Do not add post-LLM vetoes for interpretive factors that the LLM already considered.
...
- With less than `4h` left in a daily market, only open a new daily position when the strike is close, the probability is strong but below the hard cap, and Binance momentum points directly at that strike. This is LLM decision guidance, not a server-side veto after the LLM has already selected `expiry_validity: "strong"` or `"acceptable"`.
...
- After a large intraday move, do not chase the next strike unless the market consolidates/retests or Binance shows fresh continuation in the opposite or reset structure. Avoid paying for "one more push" after most of the move is already spent. This remains strategic guidance for the LLM, not a post-decision execution veto.
...
- Beecthor-bias correction: if recent Beecthor summaries remain bearish while BTC is flat or net higher over the same period, bearish `DIP` entries require explicit Binance rejection evidence (lower high, failed reclaim, support loss, downside expansion, or clear bearish repricing). Without that evidence, evaluate the nearest `REACH` momentum setup before forcing another bearish entry. This correction must be applied by the LLM while deciding; it must not be implemented as a separate server-side rejection after a valid decision is produced.
- Weekly entries are disabled. Do not open new weekly `REACH/DIP` positions under any evidence threshold.
- Account liquidity/equity is informational, not a confidence gate. Do not reject a valid setup or become more conservative merely because cash/equity is below the starting bankroll. Only hard execution constraints still apply: the requested stake must be fundable by the current cash balance and must respect stake/slot caps.
- If discarded-position unrealized losses exceed `15-20%` of current bankroll, treat that as pain-budget context and require stronger reasoning before adding exposure in the same broad direction. This is advisory context for the LLM, not a mechanical block.
