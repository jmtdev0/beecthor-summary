...
- **Daily UTC-day move cooling rule:** if BTC has already moved roughly `$1,000` from the UTC day open in one direction, do not open a fresh **daily** position in that same direction. It is rare for another clean same-direction leg to pay before same-day expiry, and these entries usually become late-chase trades. This rule does **not** block weekly markets by itself; weekly entries must still pass their own expiry, probability, closest-strike, and continuation checks.
